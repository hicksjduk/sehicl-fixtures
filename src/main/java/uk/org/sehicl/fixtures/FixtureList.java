...
{
    private final Map<String, LeagueFixtureList> leagueLists = Stream
            .of(new LeagueFixtureList("D1", 6, 2), new LeagueFixtureList("D2", 6, 2),
                    new LeagueFixtureList("D3", 10, 1), new LeagueFixtureList("D4", 10, 1),
                    new LeagueFixtureList("D5", 10, 1))
            .collect(Collectors.toMap(LeagueFixtureList::getLeague, lfl -> lfl));

    public LeagueFixtureList getLeagueList(String league)
    {
        return leagueLists.get(league);
    }

    public Collection<LeagueFixtureList> getLeagueLists()
    {
        return leagueLists.values();
    }

    public static class LeagueFixtureList
    {
        ...
        private LeagueFixtureList(String league, int teamCount, int fixtureCount)
        {
            this.league = league;
        ...
        private void generateFixtures(int fixtureCount)
        {
            ...
            IntStream.range(0, fixtureCount).forEach(
                    f -> generateFixtures(polygon, extra, f % 2 == 1));
        }
